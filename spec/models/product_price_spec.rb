...
RSpec.describe ProductPrice, type: :model do
  subject(:product_price) { FactoryBot.build_stubbed(:product_price) }

  describe '#valid' do
    it { expect(product_price).to be_valid }

    context 'when without a price' do
      subject(:product_price) { FactoryBot.build_stubbed(:product_price, price: nil) }
    ...
    context 'when without a product' do
      subject(:product_price) { FactoryBot.build_stubbed(:product_price, product: nil) }
