class SofiaAccountsController < ApplicationController
  ...
  def forgot_password
    begin
      ...
      if !sofia_account
        raise "gebruikersnaam bestaat niet"
      end

      if !sofia_account.user.email
        raise "uw account heeft geen emailadres"
      end
      ...
      param_name = I18n.t("activerecord.attributes.sofia_account.#{e.param}")
      redirect_back_or_to forgot_password_view_sofia_accounts_path, flash: { error: "Wachtwoord reset aanvragen mislukt: #{param_name.downcase} is niet aanwezig." }
    rescue StandardError => e
