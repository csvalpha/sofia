class SofiaAccountsController < ApplicationController # rubocop:disable Metrics/ClassLength
  ...
  def validate_user(user) # rubocop:disable Metrics/AbcSize, Metrics/MethodLength
    if !user
    ...
    return unless user.activation_token != activation_token || user.activation_token_valid_till.try(:<, Time.zone.now)
  ...
  def new_activation_link # rubocop:disable Metrics/AbcSize, Metrics/MethodLength
  ...
  def reset_password # rubocop:disable Metrics/AbcSize, Metrics/MethodLength
    sofia_account = SofiaAccount.find_by(id: params.require(:id))
    raise 'uw account bestaat niet' unless sofia_account
